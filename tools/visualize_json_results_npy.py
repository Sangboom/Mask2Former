...
if __name__ == "__main__":
    ...
    for dic in tqdm.tqdm(dicts):
        # if os.path.basename(dic["file_name"]) != "000000367680.jpg":
        #     continue
        # print(dic["file_name"])
        ...
        masks = [GenericMask(x, x['size'][0], x['size'][1]) for x in masks]

        # save numpy array of masks
        try:
            mask_npy = np.stack([mask._mask for mask in masks])
            ### visualize each masks ###
            # for i, mask in enumerate(mask_npy):
            #     mask = mask.astype(np.uint8)
            #     mask = cv2.resize(mask, (2448, 2048))
            #     cv2.imwrite(os.path.join(args.output, basename.replace(".jpg", f"_{i}.png")), mask*255)
        except ValueError:
            mask_npy = np.zeros((0, 2048, 2448))

        # change format as boolean
        mask_npy = mask_npy.astype(bool)
        np.save(os.path.join(args.output, basename.replace(".jpg", ".npy")), mask_npy)


        ### Visualize whole image with masks ###
