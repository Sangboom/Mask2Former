...
class Linear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0:
            ...
            self.lora_weight_u_top = nn.Parameter(self.weight.new_zeros(out_features, self.index))
            self.lora_weight_vt_top = nn.Parameter(self.weight.new_zeros(self.index, in_features))
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A'):
            ...
            nn.init.zeros_(self.lora_weight_u_top)
            nn.init.zeros_(self.lora_weight_vt_top)
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.FLAG < self.warmup:
                if self.FLAG == 0:
                    ...
                    self.lora_weight_u_top.requires_grad = False
                    self.lora_weight_vt_top.requires_grad = False
                self.FLAG += 1
                return result
                ...
                self.lora_weight_u_top.data = weight_u[:, top_index]
                self.lora_weight_vt_top.data = weight_vt[top_index, :]

                self.lora_index.requires_grad = True
                self.FLAG += 1

            if self.FLAG > self.warmup:
                result += self.lora_dropout(x) @ (self.lora_weight_u_top @ torch.diag(self.lora_index) @ self.lora_weight_vt_top).T
                return result
